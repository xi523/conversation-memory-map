...
description: Turn long, linear conversations into a traceable discussion-state map and a durable topic knowledge map. Use only when the user explicitly asks to create, sync, update, review, or preserve a conversation map, or invokes $conversation-memory-map; do not run on ordinary conversation turns.
---
